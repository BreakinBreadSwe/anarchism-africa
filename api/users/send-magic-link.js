// POST /api/users/send-magic-link
// Admin-only: send a magic-link email to any registered user.
// Body: { email }
//
// Reuses the same logic as /api/auth/email/start but bypasses the self-service
// throttle check (admin can re-send immediately).

import { put, list } from '@vercel/blob';

const ADMIN_TOKEN = process.env.AA_ADMIN_TOKEN || '';
const TTL_MS = 15 * 60 * 1000;

function isAdmin (req) {
  return ADMIN_TOKEN && req.headers['x-aa-admin-token'] === ADMIN_TOKEN;
}

function randomToken () {
  const crypto = require('node:crypto');
  return crypto.randomBytes(24).toString('base64url');
}

async function writeBlob (key, value) {
  return put(key, JSON.stringify(value), {
    access: 'public', addRandomSuffix: false, allowOverwrite: true,
    contentType: 'application/json'
  });
}

export default async function handler (req, res) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'POST only' });
  if (!isAdmin(req))         return res.status(401).json({ error: 'Unauthorized' });

  const email = String((req.body || {}).email || '').trim().toLowerCase();
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
    return res.status(400).json({ error: 'invalid email' });

  if (!process.env.RESEND_API_KEY)
    return res.status(500).json({ error: 'RESEND_API_KEY not set' });
  if (!process.env.RESEND_FROM_EMAIL)
    return res.status(500).json({ error: 'RESEND_FROM_EMAIL not set' });

  const token = randomToken();
  await writeBlob('auth/email-tokens/' + token + '.json', {
    email, expires: Date.now() + TTL_MS, used: false
  });

  const origin = process.env.SITE_URL || `https://${req.headers.host || 'anarchism.africa'}`;
  const link   = `${origin}/api/auth/email/verify?token=${encodeURIComponent(token)}`;

  const r = await fetch('https://api.resend.com/emails', {
    method: 'POST',
    headers: { 'Authorization': 'Bearer ' + process.env.RESEND_API_KEY, 'Content-Type': 'application/json' },
    body: JSON.stringify({
      from:    process.env.RESEND_FROM_EMAIL,
      to:      [email],
      subject: 'Your sign-in link — ANARCHISM.AFRICA',
      html: `<!doctype html><html><body style="background:#0a0a0a;color:#f5f0e8;font-family:'Helvetica Neue',Arial,sans-serif;padding:32px;margin:0">
  <div style="max-width:520px;margin:0 auto;background:#111;border:1px solid #222;border-radius:0;padding:32px">
    <h1 style="font-family:'Bebas Neue','Helvetica Neue',Arial,sans-serif;letter-spacing:.04em;font-size:1.6rem;margin:0 0 18px;color:#f5f0e8">ANARCHISM.AFRICA</h1>
    <p style="font-size:1rem;line-height:1.6;color:#cfc8bf;margin:0 0 22px">You've been invited to sign in. This link is good for 15 minutes and can only be used once.</p>
    <p style="margin:0 0 26px"><a href="${link}" style="display:inline-block;background:#f5f0e8;color:#0a0a0a;padding:14px 22px;text-decoration:none;font-weight:700;letter-spacing:.06em;border-radius:0">Sign in</a></p>
    <p style="font-family:'JetBrains Mono',monospace;font-size:.72rem;color:#9c948e;word-break:break-all;margin:0 0 22px">${link}</p>
    <p style="font-size:.72rem;color:#7a7370">If you didn't expect this, ignore the email — nothing happens unless you click.</p>
  </div>
</body></html>`,
      text: `Sign in to ANARCHISM.AFRICA\n\n${link}\n\n(Valid 15 min, single use.)`
    })
  });

  if (!r.ok) {
    const d = await r.json().catch(() => ({}));
    return res.status(502).json({ error: 'send failed: ' + (d.message || r.status) });
  }
  res.status(200).json({ ok: true });
}
