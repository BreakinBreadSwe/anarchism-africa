...
function buildQS (q = {}) {
  ...
  if (q.order) {
    // Two accepted forms per token, comma-separated:
    //   1. Shorthand: '-col' = desc, 'col' = asc — we append the direction.
    //   2. Already Postgrest-formatted: 'col.desc' / 'col.asc.nullslast' /
    //      'col.desc.nullsfirst' — passed through untouched.
    // If a caller passes 'scraped_at.desc.nullslast', appending '.asc' would
    // produce 'scraped_at.desc.nullslast.asc' → Postgrest PGRST100 error,
    // which is exactly what broke the content list endpoint. Detect the
    // Postgrest form by presence of any '.' AFTER position 0.
    const o = q.order.split(',').map(s => {
      s = s.trim();
      if (!s) return s;
      if (s.indexOf('.') > 0) return s;  // already Postgrest-formatted
